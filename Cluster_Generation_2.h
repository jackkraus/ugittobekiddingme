...
void insert_cluster(int g[nrow][ncol]){
    ...
    int hitval1 = rand() % hitmaxval;
    g[x][y] = hitval1;
    // cout<<"hitval1 is "<<hitval1<<endl;
    ...
    for(unsigned int i=0; i<hitnum; i++){
        ...
        if(hitloc == 0){
            int x_h=x-1;
            int y_h=y-1;
            int hitval = rand() % hitval1;
            // cout<<"hitval is "<<hitval<<endl;
            g[x_h][y_h]=hitval;
        }
        if(hitloc == 1){
            int x_h=x-1;
    ...
    if(f==0){
        for(unsigned int i=0; i<nrow; i++){
            for(unsigned int j=0; j<ncol; j++){
                if(tg[i][j]!=0){
                    g[i][j]=tg[i][j];
                    // cout<<"element in temp grid is "<<tg[i][j]<<endl;
                }
            }
        }
    }
