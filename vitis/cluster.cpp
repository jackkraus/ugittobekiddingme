...
void naive_findClustersHLS2(bit_t grid[NROWS + 2][NCOLS + 2], Point pointsGrid[NROWS + 2][NCOLS + 2]) {
    id_t maxClusterID = 1;
    id_t clusterGrid1[NROWS + 2][NCOLS + 2] = {0};
    id_t clusterGrid2[NROWS + 2][NCOLS + 2] = {0};
...
#pragma HLS PIPELINE II=11
            if (grid[i][j]) {
                id_t minClusterID = maxClusterID;
				id_t neighborID0 = clusterGrid1[i + upperLeftNeighborXCoords[0]][j + upperLeftNeighborYCoords[0]];
				id_t neighborID1 = clusterGrid1[i + upperLeftNeighborXCoords[1]][j + upperLeftNeighborYCoords[1]];
...
#pragma HLS PIPELINE II=1
            if (grid[i][j]) {
                id_t clusterID = clusterGrid2[i][j] ? clusterGrid2[i][j] : clusterGrid1[i][j];
