...
void naive_findClustersHLS2(int grid[NROWS + 2][NCOLS + 2], Point pointsGrid[NROWS + 2][NCOLS + 2]) {
    int maxClusterID = 1;
    int clusterGrid1[NROWS + 2][NCOLS + 2] = {0};
    int clusterGrid2[NROWS + 2][NCOLS + 2] = {0};
...
#pragma HLS PIPELINE II=11
            if (grid[i][j]) {
                int minClusterID = maxClusterID;
                int neighborIDs[4] = {
					clusterGrid1[i + upperLeftNeighborXCoords[0]][j + upperLeftNeighborYCoords[0]],
					clusterGrid1[i + upperLeftNeighborXCoords[1]][j + upperLeftNeighborYCoords[1]],
					clusterGrid1[i + upperLeftNeighborXCoords[2]][j + upperLeftNeighborYCoords[2]],
					clusterGrid1[i + upperLeftNeighborXCoords[3]][j + upperLeftNeighborYCoords[3]]
                };

				if (neighborIDs[0] && minClusterID > neighborIDs[0]) minClusterID = neighborIDs[0];
				if (neighborIDs[1] && minClusterID > neighborIDs[1]) minClusterID = neighborIDs[1];
				if (neighborIDs[2] && minClusterID > neighborIDs[2]) minClusterID = neighborIDs[2];
				if (neighborIDs[3] && minClusterID > neighborIDs[3]) minClusterID = neighborIDs[3];
...
#pragma HLS PIPELINE II=11
            if (grid[i][j]) {
                int minID = clusterGrid1[i][j];
                int neighborIDs[4] = {
					clusterGrid1[i + lowerRightNeighborXCoords[0]][j + lowerRightNeighborYCoords[0]],
					clusterGrid1[i + lowerRightNeighborXCoords[1]][j + lowerRightNeighborYCoords[1]],
					clusterGrid1[i + lowerRightNeighborXCoords[2]][j + lowerRightNeighborYCoords[2]],
					clusterGrid1[i + lowerRightNeighborXCoords[3]][j + lowerRightNeighborYCoords[3]],

                };

                if (neighborIDs[0] && neighborIDs[0] < minID) minID = neighborIDs[0];
                if (neighborIDs[1] && neighborIDs[1] < minID) minID = neighborIDs[1];
                if (neighborIDs[2] && neighborIDs[2] < minID) minID = neighborIDs[2];
                if (neighborIDs[3] && neighborIDs[3] < minID) minID = neighborIDs[3];

                clusterGrid2[i][j] = minID;
                clusterGrid2[i + lowerRightNeighborXCoords[0]][j + lowerRightNeighborYCoords[0]] = minID;
				clusterGrid2[i + lowerRightNeighborXCoords[1]][j + lowerRightNeighborYCoords[1]] = minID;
				clusterGrid2[i + lowerRightNeighborXCoords[2]][j + lowerRightNeighborYCoords[2]] = minID;
				clusterGrid2[i + lowerRightNeighborXCoords[3]][j + lowerRightNeighborYCoords[3]] = minID;
            }
        }
    }
...
#pragma HLS PIPELINE II=1
            if (grid[i][j]) {
                int clusterID = clusterGrid2[i][j] ? clusterGrid2[i][j] : clusterGrid1[i][j];
